...
export function yearNums(currentYear){
    let years = []
    for(let index=-12;index<13;index++){
        years.push(currentYear.clone().addYears(index))
    }
    return years
}

//年份区间的第一个
export function calcYearRange(firstYear){
    let years = []
    for(let index=-25;index<0;index++){
        years.push(firstYear.clone().addYears(index))
    }
    return years
}

export function monthNums(currentMonth) {
    let monthArray = []
    let month = currentMonth.getMonth()

    for(let index=month;index>=0;index--){
        monthArray.push(currentMonth.clone().addMonths(-index))
    }
    // monthArray.push(currentMonth)
    for(let index=1;index<=11-month;index++){
        monthArray.push(currentMonth.clone().addMonths(index))
    }
    return monthArray
}
